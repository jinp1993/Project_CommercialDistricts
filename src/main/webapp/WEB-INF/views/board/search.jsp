			...
			<div class="row">
				<div class=" fh5co-service1">
					<div class="fh5co-desc2">
						...
						<h3><a href="#" class="m-tcol-t1" title="검색내용">Search Resultse</a>
						...
						<form action="search" method="post" style="text-align: center;">
							...
							<ul class="ul">
								<c:if test="${pageMaker.prev}">
									<li><a class="title"
										href="search?board=search&page=${pageMaker.starPage - 1}">이전</a></li>
								</c:if>
								<c:forEach begin="${pageMaker.starPage}"
										...
										<li
											...
											<a class="title" href="search?board=search&sel=${map.sel}&search=${map.search}&page=${idx}">${idx}</a>
										</li>
								
								</c:forEach>
								<c:if test="${pageMaker.next && pageMaker.endPage > 0 }">
									<li><a class="title"
										href="search?board=search&sel=${map.sel}&search=${map.search}&page=${pageMaker.endPage + 1 }">다음</a></li>
								</c:if>
							</ul>
						</div>
					</div>
