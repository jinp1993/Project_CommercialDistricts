			...
			<div class="row">
				<div class=" fh5co-service1">
					<div class="fh5co-desc2">
						...
						<h3>
							<a href="Free_Board" class="m-tcol-t1" title="공지사항">Free_Board</a>
						</h3>
							...
							<c:forEach items="${list}" var="dto">
								...
								<c:if test="${dto != null}">
									<tr>
										...
										<td>${dto.name}</td>
										<td><fmt:formatDate
													value="${dto.savedate}" pattern="yyyy-MM-dd  HH:mm" /></td>
										<td>${dto.hit }</td>
									</tr>
								</c:if>
						...
						<div align="center">
							<ul class="ul">
								<c:if test="${pageMaker.prev}">
									<li><a class="title"
										href="Free_Board?board=Free_Boarde&page=${pageMaker.starPage - 1}">이전</a></li>
								</c:if>
								<c:forEach begin="${pageMaker.starPage}"
									end="${pageMaker.endPage}" var="idx">
									<li
										<c:out value="${pageMaker.criteria.page == idx ? 'class=active':'d' }"/>>
										<a class="title" href="Free_Board?board=Free_Board&page=${idx}">${idx}</a>
									</li>

								</c:forEach>
								<c:if test="${pageMaker.next && pageMaker.endPage > 0 }">
									<li><a class="title"
										href="Free_Board?board=Free_Board&page=${pageMaker.endPage + 1 }">다음</a></li>
								</c:if>
							</ul>
						</div>
					</div>
